...
List squash_by_cpp(
  ...
  // TODO: Use `R_xlen_t` where appropriate
  int n_groups = group_locs.size();
  const int* p_size = INTEGER(size);
  SpanSetBuffer buffer(n_groups);

  std::vector<double> temp_group_starts;
  std::vector<double> temp_group_ends;
  temp_group_starts.reserve(16);
  temp_group_ends.reserve(16);
  ...
  for (int group = 0; group < n_groups; group++) {
    ...
    std::vector<size_t> span_indices(temp_group_starts.size());
    std::iota(span_indices.begin(), span_indices.end(), 0);
    ...
    for (size_t i = 1; i < span_indices.size(); i++) {
